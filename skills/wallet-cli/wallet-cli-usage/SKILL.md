...
description: Official Ledger wallet-cli — USB-based CLI for Ledger hardware wallet flows (account discover, receive, balances, operations, send, swap quote/execute/status, genuine-check). Use for any wallet-cli command execution and for mapping informal requests ("show me my wallet", "send some ETH", "is this Ledger real") to the right command.
---
...
| "where do I send funds to", "give me my address", "deposit address" | `receive <label>` |
| "how much do I have", "balance", "what's my ETH balance" | `balances <label>` |
| "what did I send", "transaction history", "recent activity" | `operations <label>` |
| "send X to Y", "transfer", "pay", "withdraw to an exchange" | `send <label> --to <addr> --amount '<n> <TICKER>'` |
...
- **Staking** (Solana validators, ETH staking, Cosmos delegation).
- **NFTs** (mint, transfer, view).
- **Encryption / OpenPGP / key-share operations.**
- **`send`, `receive`, `operations`, or `swap execute` on testnets** — see the Networks matrix above.
- **Custom chains** not listed in the Networks matrix above.
...
```
For the full list of network IDs and which commands each supports, see the Networks section above.

### receive
```bash
wallet-cli receive ethereum-1
wallet-cli receive ethereum-1 --no-verify  # skip device confirmation
```
...
wallet-cli genuine-check --output json  # only if a downstream caller needs to parse the result
```
...
**Addresses (pick one per side):** `--from-fresh-address` or `--from-account`; `--to-fresh-address` or `--to-account`. Account flags accept a session label; the CLI resolves a fresh receive address like `receive`.

```bash
wallet-cli swap quote --from ethereum --to bitcoin --amount 0.1 --from-fresh-address 0xABC... --to-fresh-address bc1q...
wallet-cli swap quote -f ethereum -t bitcoin --amount 0.1 --from-fresh-address 0xABC... --to-fresh-address bc1q... --output json
wallet-cli swap quote --from ethereum --to bitcoin --amount 0.1 --from-account ethereum-1 --to-account bitcoin-native-1
wallet-cli swap quote --from ethereum --to bitcoin --amount 0.1 --from-account ethereum-1 --to-account bitcoin-native-1 --output json
```
